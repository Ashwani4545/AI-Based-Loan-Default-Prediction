...
### 🗂️ Confusion Matrix (from `model_metrics.json`)
...
webapp/app.py  (Flask-SocketIO + Eventlet)
  ...
  ├─ 4.  add_economic_features()      ← macro context (constants)
...
### 7️⃣ (Optional) SHAP Plots + Fairness Report
...
### 8️⃣ (Optional) Evaluate Saved Model
...
**🎚️ Decision threshold = 0.3445.** Lower than 0.5, deliberately. In a lending context, a False Negative (missed defaulter) carries a higher cost than a False Positive (rejected good borrower). The threshold is set at the value that maximised profit on the validation set.

**⚖️ SMOTE + `scale_pos_weight`.** Two independent mechanisms for class imbalance — SMOTE at the data level, `scale_pos_weight` at the model level — giving the minority class sufficient signal without over-dependence on either technique alone.
...
**🗄️ Flat JSON for storage.** `prediction_history.json` is capped at 500 entries. `audit_log.json` is append-only. Both are intentional for a single-machine deployment; a production multi-user deployment would replace these with a relational database.
...
**Ashwani Pandey** — MCA Final Year Project  
